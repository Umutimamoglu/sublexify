...
const MediaDetailPage = () => {
    ...
    const handleToggleKnown = async (wordId: number, currentStatus: boolean) => {
        if (!isAuthenticated) {
            openLoginModal(t('auth.loginRequiredWord', 'Kelimeleri öğrenildi olarak işaretlemek için giriş yapmalısınız.'));
            return;
        }
        try {
            if (currentStatus) {
    ...
    const handleMarkBatchAsKnown = async () => {
        if (!isAuthenticated) {
            openLoginModal(t('auth.loginRequiredBatch', 'Toplu işlem yapabilmek için giriş yapmalısınız.'));
            return;
        }
    ...
    const handleGenerateList = async () => {
        if (!isAuthenticated) {
            openLoginModal(t('auth.loginRequiredList', 'Liste oluşturabilmek için giriş yapmalısınız.'));
            return;
        }
