...
if __name__ == "__main__":
    # Test for S01E02 (Cherry)
    srt_path = "/Users/umutimamoglu/sublex/subtitles/The.Boys.S01E02.srt"
    db_path = "/Users/umutimamoglu/sublex/backend/the_boys_s1e2_db_words_final.txt"
    ...
    print(f"DB unique words (Fresh S01E02): {len(db_words)}")
    
    # Mismatch summary
    only_srt = set(srt_words.keys()) - set(db_words.keys())
    only_db = set(db_words.keys()) - set(srt_words.keys())
    
    print(f"\nUnique to SRT: {len(only_srt)}")
    print(f"Unique to DB: {len(only_db)}")
    
    if only_srt:
        print("\nTop 10 MISSING in DB:")
    ...
    # Count check for specific words
    check_list = ["i", "don't", "i'm", "a-train", "popclaw"]
    print("\nWord Count Verification:")
    for w in check_list:
        srt_c = srt_words.get(w, 0)
        db_c = db_words.get(w, 0)
        status = "MATCH ✅" if srt_c == db_c else "MISMATCH ❌"
